...
public:
	...
	inline float32 Compute(const PColor& col_p,const PGradient& grad_p, const sint32& x, const sint32& y, const float32& d) const
	{
		...
		// ��ɫ�ռ����
		const PVector3f col_q = GetColor(img_right_, xr, y);
		const float dc = std::min(fabs((float)col_p.b - (float)col_q.x) + fabs((float)col_p.g - (float)col_q.y) + fabs((float)col_p.r - (float)col_q.z), tau_col_);

		// �ݶȿռ����
		const PVector2f grad_q = GetGradient(grad_right_, xr, y);
	...
	inline float32 ComputeA(const sint32& x, const sint32& y, const DisparityPlane& p) const
	{
		...
		const PColor& col_p = GetColor(img_left_, x, y);
		float32 cost = 0.0f;
