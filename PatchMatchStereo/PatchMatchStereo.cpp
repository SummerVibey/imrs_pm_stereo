...
bool PatchMatchStereo::Match(const uint8* img_left, const uint8* img_right, float32* disp_left)
{
	...
	// ����Ҷ�ͼ
	std::cout << "Compute Gray" << std::endl;
	ComputeGray();
	...
	// ƽ��ת�����Ӳ�
	std::cout << "Plane To Disparity" << std::endl;
	PlaneToDisparity();

	// ����һ���Լ��
	if (option_.is_check_lr) {
		// һ���Լ��
		std::cout << "LRCheck" << std::endl;
		LRCheck();
	}

	// �Ӳ����
	if (option_.is_fill_holes) {
		std::cout << "Fill Holes In DispMap" << std::endl;
		FillHolesInDispMap();
	}
...
void PatchMatchStereo::RandomInitialization() const
{
	...
	for (int k = 0; k < 2; k++) {
        ...
        // float view_vector[3];
        // const float& fx = 400, &fy = 400, &cx = (float)width / 2, &cy = (float)height / 2;

        // view_vector[0] = (x - cx) / fx;
        // view_vector[1] = (y - cy) / fy;
        // view_vector[2] = 1;

        // float dp = norm.x * view_vector[0] + norm.y * view_vector[1] + norm.z * view_vector[2];
        // if(dp > 0) {
        //   norm.x = -norm.x;
        //   norm.y = -norm.y;
        //   norm.z = -norm.z;
  ...
  cv::Mat disp_left(height, width, CV_32F, cv::Scalar(0));
  memcpy((float*)disp_left.data, disp_left_, sizeof(float)*width*height);
  cv::Mat disp_display;
  ConvertDisparityForDisplay(disp_left, disp_display);
  cv::Mat_<cv::Vec3f> norm_img(height, width);
  for(int i = 0; i < height; ++i) {
    for(int j = 0; j < width; ++j) {
      PVector3f norm = plane_left_[i * width + j].to_normal();
      norm_img(i, j) = cv::Vec3f(norm.x, norm.y, norm.z);
    }
  }
  cv::Mat norm_display;
  ConvertNormalsForDisplay(norm_img, norm_display);

  cv::imshow("disp_left", disp_display);
  cv::imshow("norm left",norm_display);
  while (1)
  { 
    if(cv::waitKey(0) == 'q') break; /* code */
  }
...
void PatchMatchStereo::ComputeGradient() const
{
  ...
  cv::Mat gradx_img(height, width, CV_32F, cv::Scalar(0)), grady_img(height, width, CV_32F, cv::Scalar(0));
	// Sobel�ݶ�����
	for (sint32 n = 0; n < 2; n++) {
		auto* gray = (n == 0) ? gray_left_ : gray_right_;
        ...
        if(n == 0) {
          gradx_img.ptr<float>(y)[x] = grad[y * width + x].x;
          grady_img.ptr<float>(y)[x] = grad[y * width + x].y;
        }
          
			}
		}
	}
  cv::Mat gradx_display, grady_display;
  double max_val, min_val;
  cv::Point max_loc, min_loc;
  cv::minMaxLoc(gradx_img, &min_val, &max_val, &min_loc, &max_loc);
  printf("max cost = %f, min cost = %f\n", max_val, min_val);
  gradx_img.convertTo(gradx_display, CV_32F, 1.f/(max_val-min_val),-min_val/(max_val-min_val));

  cv::minMaxLoc(grady_img, &min_val, &max_val, &min_loc, &max_loc);
  printf("max cost = %f, min cost = %f\n", max_val, min_val);
  grady_img.convertTo(grady_display, CV_32F, 1.f/(max_val-min_val),-min_val/(max_val-min_val));

  cv::imshow("gradx", gradx_display);
  cv::imshow("grady", grady_display);
  while(1) {
    if(cv::waitKey(0) == 'q') break;
  }
}
