...
__device__ __forceinline__ void GetHomography(
  const float *K_ref, 
  const float *K_src, 
  const float *R1,
  const float *R2,
...
)
{
  ...
  // Distance to the plane.
  const float dist = normal.x * point3d[0] + normal.y * point3d[1] + normal.z * point3d[2];
  const float inv_dist = 1.0f / dist;
  const float inv_dist_N0 = inv_dist * normal.x;
  const float inv_dist_N1 = inv_dist * normal.y;
  const float inv_dist_N2 = inv_dist * normal.z;
...
{
  return expf(-(color_diff * color_diff) / (2 * sigma_color * sigma_color));
  // return expf( -(x_diff * x_diff + y_diff * y_diff) / (2 * sigma_spatial * sigma_spatial) 
  //             -(color_diff * color_diff) / (2 * sigma_color * sigma_color));
}

__device__ __forceinline__ float CurandBetween(curandState *curand_map, float min, float max)
{
  return (curand_uniform(curand_map) * (max-min) + min);
}
  ...
  bool is_right)
{
  ...
  RandomPointPlane(&plane[center], &curand_map[center], x, y, height, width, options->min_disparity, options->max_disparity, K);
}
...
)
{
  ...
  const float dg = 0.0625f * fminf(fabs(grad_px - grad_qx)+ fabs(grad_py - grad_qy), options->tau_grad);
...
)
{
  ...
  // float qcx, qcy;
  // TransformView(Hsr, px, py, qcx, qcy);
  ...
  // return fminf(kMaxCost, cost);
  return cost;
}

__device__ __forceinline__ float ComputeNCC(
  RefViewType *view_ref, 
  ViewType *view_src,
...
)
{
  ...
  float Hsr[9];
  GetHomography(view_ref->params_->K_,
                view_src->params_->K_,
                view_ref->params_->R_,
                view_src->params_->R_,
                view_ref->params_->t_,
  ...
  float ref_mean = 0, src_mean = 0;
  int window_area = 0;
  for (int y_bias = -window_radius; y_bias <= window_radius; y_bias += options->step_size) {
    for (int x_bias = -window_radius; x_bias <= window_radius; x_bias += options->step_size) {
  ...
  ref_mean /= window_area;
  src_mean /= window_area;

  float ref_src_color_sum = 0, ref_color_squared_sum = 0, src_color_squared_sum = 0;
  for (int y_bias = -window_radius; y_bias <= window_radius; y_bias += options->step_size) {
    for (int x_bias = -window_radius; x_bias <= window_radius; x_bias += options->step_size) {
      ...
      float bilateral_weight = BilateralWeight(x_bias, y_bias, ref_color - ref_center_color, options->sigma_spatial, options->sigma_color);
      bilateral_weight = 1;
      const float ref_value = bilateral_weight * (ref_color - ref_mean);
      const float src_value = bilateral_weight * (src_color - src_mean);

      ref_src_color_sum += ref_value * src_value;
      ref_color_squared_sum += ref_value * ref_value;
      src_color_squared_sum += src_value * src_value;
    }
  }
...
)
{
  ...
  for (int y_bias = -window_radius; y_bias <= window_radius; y_bias += options->step_size) {
    for (int x_bias = -window_radius; x_bias <= window_radius; x_bias += options->step_size) {
      ...
      ref_color_sum += bilateral_weight * ref_color;
      src_color_sum += bilateral_weight * src_color;
      ref_color_squared_sum += bilateral_weight * ref_color * ref_color;
      src_color_squared_sum += bilateral_weight * src_color * src_color;
      src_ref_color_sum += bilateral_weight * ref_color * src_color;
...
__device__ __forceinline__ void PropagateSpatialBase(
  RefViewType *view_ref,
  ViewType *view_src,
  PlaneState *plane, 
  float *cost, 
...
)
{
  ...
  int trial_num = 0;
    ...
    float idp_rd = CurandBetween(&curand_map[center_local], -1.f, 1.f) * disp_update;
    ...
    norm_rd.x = CurandBetween(&curand_map[center_local], -1.f, 1.f) * norm_update;
    norm_rd.y = CurandBetween(&curand_map[center_local], -1.f, 1.f) * norm_update;
    float z = CurandBetween(&curand_map[center_local], -1.f, 1.f) * norm_update;
    while (z == 0.0f) {
      z = CurandBetween(&curand_map[center_local], -1.f, 1.f) * norm_update;
    }
    norm_rd.z = z;
...
)
{
  ...
  for(int i = 0; i < 8; ++i) {
    PropagateSpatialBase(view_ref, view_src, plane, cost, x, y, x_bias[i], y_bias[i], options, height, width);
    __syncthreads();
  }

  RefinePlaneBase(view_ref, view_src, plane, cost, curand_map, x, y, options, height, width);
  __syncthreads();
}

__global__ void PropagateBlackBase(
  RefViewType *view_ref,
  ViewType *view_src,
  PlaneState *plane, 
  float *cost, 
...
)
{
  ...
  if(x < 0 || x > width - 1 || y < 0 || y > height - 1) return;
  PropagateBase(view_ref, view_src, plane, cost, curand_map, x, y, options, height, width);
}

__global__ void PropagateRedBase(
  RefViewType *view_ref,
  ViewType *view_src,
  PlaneState *plane, 
  float *cost, 
...
)
{
  ...
  // view_ref->cost_map_[center] = ComputePhotoConsistencyCost(
...
__global__ void RandomInitKernel(curandState *curand_map, unsigned long long seed, int height, int width)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x; 
  int y = threadIdx.y + blockIdx.y * blockDim.y; 
  if(x < 0 || x > width - 1 || y < 0 || y > height - 1) return;
  ...
  int center = y * width + x;
  curand_init(seed, y, x, &curand_map[center]);
}

__device__ __forceinline__ float PerturbDepth(
  const float depth,
...
{
  const float max = fminf(depth + perturbation, max_depth);
  const float min = fmaxf(depth - perturbation, min_depth);
  
  return CurandBetween(curand, min, max);
}


__global__ void RandomDepthKernel(
  float *depth_map,
  curandState *curand_map,
  const float min_depth,
  const float max_depth,
...
)
{
  ...
  depth_map[center] = CurandBetween(&curand_map[center], min_depth, max_depth);
  __syncthreads();
}
...
)
{
  ...
  float3 norm_rd;
  norm_rd.x = CurandBetween(curand, -0.5f, 0.5f) * perturbation;
  norm_rd.y = CurandBetween(curand, -0.5f, 0.5f) * perturbation;
  norm_rd.z = CurandBetween(curand, -0.5f, 0.5f) * perturbation;

  perturbed_normal->x = normal.x + norm_rd.x;
  perturbed_normal->y = normal.y + norm_rd.y;
  perturbed_normal->z = normal.z + norm_rd.z;

  // Make sure normal has unit norm.
  const float inv_norm = rsqrtf(perturbed_normal->x *  perturbed_normal->x + \
  ...
  float dp = perturbed_normal->x * view_ray.x + perturbed_normal->y * view_ray.y + perturbed_normal->z * view_ray.z;
  if (dp >= 0.0f) {
    perturbed_normal->x = -perturbed_normal->x;
    perturbed_normal->y = -perturbed_normal->y;
    perturbed_normal->z = -perturbed_normal->z;
  }
}
...
__global__ void RandomNormalKernel(
  float3 *normal_map,
  curandState *curand_map, 
  int height, int width,
  const float *kmat
)
{
  ...
  int center = x + y * width;
  GenerateRandomNormal(&normal_map[center], &curand_map[center], x, y, kmat);
  __syncthreads();
}

__global__ void CostInitKernel(
  RefViewType *view_ref,
  ViewType *view_src,
  const float *depth_map,
  const float3 *normal_map,
  float *cost_map,
  const PatchMatchOptions *options,
  int height, int width
)
{
  ...
  int center = x + y * width;
  // cost_map[center] = ComputePMCostRegion(view_ref, view_src, x, y, depth_map[center], normal_map[center], options, height, width);
  // cost_map[center] = ComputeBilateralNCC(view_ref, view_src, x, y, depth_map[center], &normal_map[center*3], options, height, width);
  cost_map[center] = ComputeNCC(view_ref, view_src, x, y, depth_map[center], normal_map[center], options, height, width);
  __syncthreads();
}
...
)
{
  ...
  cost_map[center] = \
    ComputeMultiViewPMCost(view_ref, view_src, src_size, x, y, depth_map[center], normal_map[center], options, height, width);
  __syncthreads();
}
...
)
{
  ...
  if(cost_nb < cost_lc) {
    cost_lc = cost_nb;
    depth_lc = depth_nb;
    normal_lc.x = normal_nb.x;
    normal_lc.y = normal_nb.y;
    normal_lc.z = normal_nb.z;
  }
}
...
{
  const float max_depth = static_cast<float>(options->max_depth);
  ...
  // int trial_num = 0;
    ...
    cost_new = ComputeNCC(view_ref, view_src[0], x, y, depth_pt, normal_lc, options, height, width);
    // cost_new = \
    ...
    cost_new = ComputeNCC(view_ref, view_src[0], x, y, depth_lc, normal_pt, options, height, width);
    // cost_new = \
    ...
    if(cost_new < cost_lc) {
      cost_lc = cost_new;
      normal_lc.x = normal_pt.x;
      normal_lc.y = normal_pt.y;
      normal_lc.z = normal_pt.z;
    }
    ...
    cost_new = ComputeNCC(view_ref, view_src[0], x, y, depth_pt, normal_pt, options, height, width);
    // cost_new = \
...
void MVSMatcherCuda::Match(cv::Mat &depth, cv::Mat &normal)
{
  ...
  RandomInitKernel <<<grid_size, block_size>>>(ref_->curand_map_, ts.tv_nsec, height, width);
  cudaDeviceSynchronize();
  GenerateRandomState <<<grid_size, block_size>>>(ref_->plane_, ref_->curand_map_, options_, height, width, ref_->params_->K_, false);
  // RandomPlaneInOrder <<<1, 1>>> (ref_->plane_, ref_->curand_map_, options_, height, width, ref_->params_->K_, false);
  cudaDeviceSynchronize();
  ...
  for(int iter = 0; iter < 20; ++iter) {
    PropagateRedBase<<<grid_size, block_size>>>(ref_, src_[0], ref_->plane_, ref_->cost_map_, ref_->curand_map_, options_, height, width);
    cudaDeviceSynchronize();
    PropagateBlackBase<<<grid_size, block_size>>>(ref_, src_[0], ref_->plane_, ref_->cost_map_, ref_->curand_map_, options_, height, width);
    cudaDeviceSynchronize();
    ShowDepthAndNormal(ref_->plane_, height, width);
  }
...
void MVSMatcherCuda::Match(int size)
{
  int block_size_x = 16;
  int block_size_y = 16;
  const int width = ref_->width_;
  ...
  clock_gettime(CLOCK_REALTIME, &ts);
  RandomInitKernel <<<grid_size, block_size>>>(ref_->curand_map_, ts.tv_nsec, height, width);
  RandomDepthKernel <<<grid_size, block_size>>>(ref_->depth_map_, ref_->curand_map_, options_->min_depth, options_->max_depth, height, width);
  ...
  for(int iter = 0; iter < 8; ++iter) {
    ...
    cudaDeviceSynchronize();
    RenderDepthAndNormalMap(ref_->depth_map_, (float*)ref_->normal_map_, height, width);
  }
